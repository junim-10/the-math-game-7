...
from DadosFuncionais import DadosFuncionais
...
class TelaJogo:
    ...
    def atualizarTempo(self):
        ...
        minutos = elapsed // 60
        segundos = elapsed % 60
        if self.tempo_label and self.tempo_label.winfo_exists(): # Verifique se o widget ainda existe
            self.tempo_label.config(text=f"{minutos:02d}:{segundos:02d}")
            self.timer_id = self.root.after(1000, self.atualizarTempo)
        else:
            self.pararTimer() # Se não existir, pare o timer
